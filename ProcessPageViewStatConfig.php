...
// Whois
    ...
    $field = $this->modules->get('InputfieldSelect');
    $field->name = 'whois_mode';
    $field->label = __('External/Internal');
    $field->description = __('Opens the specified Whois service when clicking on the IP address.');
    $field->required = true;
    $field->addOptions(array(
      0 => __('Internal WHOIS'),
      1 => __('External WHOIS')
    ...
    $field->label = __('External WHOIS');
    $field->description = __('Specify a external WHOIS service. Note the place holder %s for the IP address. For example: https://whatismyipaddress.com/ip/%s');
    $fieldset->add($field);
...
// About
    ...
    $field = $this->modules->get('InputfieldMarkup');
    $field->value = '<p style="text-align:center">'.__('This module uses:').' <a target="_blank" href="https://www.ip2location.com/">IP2Location</a>, <a target="_blank" href="http://github.com/asvd/dragscroll">dragscroll</a> and <a target="_blank" href="https://www.toms-world.org/blog/parseuseragentstring/">User-Agent-Parser</a>.</p>
