...
function showIPInfo($ip, $module_url, $latitude, $longitude) {
  echo '<!DOCTYPE html>';
  echo '<html>';
  echo '<head>';
  ...
  if ($ip != '') {
    ...
    if (($latitude != '') && ($longitude != '')) {
      ...
      echo 'var feature=new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point('.$longitude.','.$latitude.').transform(epsg4326,projectTo));';
      ...
      echo 'map.setCenter(new OpenLayers.LonLat('.$longitude.','.$latitude.').transform(epsg4326,projectTo),4);';
      echo '</script>';
    }
  }
